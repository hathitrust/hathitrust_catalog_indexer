...
class CLI < Thor
  ...
  def index_date(date_string, logfile = nil)
    ...
    if File.exist? delfile
      ...
      logger.fatal "MISSING $SCRIPTDIR/delete_ids.rb"
      # FIXME: missing script file
      # delout=`$SCRIPTDIR/delete_ids $delfile`
    else
  ...
  def marc_file_for_date(date_string)
    File.join(data_directory, "zephir_upd_#{date_string}.json.gz")
  end

  def del_file_for_date(date_string)
    File.join(data_directory, "zephir_upd_#{date_string}_delete.txt.gz")
  end

  def run_traject(marcfile, reader = default_reader, writer = default_writer)
    ...
    setup_environment
    if File.exist? writer_path(writer)
      logger.debug "writer #{writer} at #{writer_path(writer)}"
    else
      fatal "Can't find writer #{writer_path(writer)}"
    end
    update_collection_map
    call_traject_binary(marcfile, reader, writer)
  end

  def call_traject_binary(marcfile, reader, writer)
    logger.info "Indexing from #{marcfile} into #{writer} (#{ENV["SOLR_URL"]})"
  ...
  # Returns full path to the appropriate Ruby file.
  # Example: writer_path("localhost") => "/app/writers/localhost.rb"
  def writer_path(name = default_writer)
    File.join(hathitrust_catalog_indexer_path, "writers", name + ".rb")
  end
