...
class Traject::GZipXMLReader < Traject::Marc4JReader
  def create_marc_reader!
    ...
    when 'xml'
      @input_stream = if @settings['command_line.filename'] =~ /\.gz$/
                        Java::java.util.zip.GZIPInputStream.new(@input_stream.to_inputstream)
                      else
                        @input_stream.to_inputstream
                      end

      MarcXmlReader.new(@input_stream)
    else
      raise IllegalArgument, "Unrecgonized marc_source.type: #{input_type}"
    end
  end
end
