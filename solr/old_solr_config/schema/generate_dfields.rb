...
def dfield(fname, type, indexed, multiple, stored = false)
  ...
  s = stored ? 'true' : 'false'
  %(<dynamicField name="*_#{fname}" type="#{type}" indexed="#{i}" stored="#{s}" multiValued="#{m}" />)
end
...
def storedMulti_copyfield(fname)
  %(<copyField source="*_#{fname}" dest="*_a" />)
end

def storedSingle_copyfield(fname)
  %(<copyField source="*_#{fname}" dest="*" />)
end

def copyfield(fname, bt)
  %(<copyField source="*_#{fname}" dest="*_#{bt}" />)
end
...
puts %(<copyField source="*_ssort_stored" dest="*_sort" />)
puts %(<copyField source="*_ssort_stored" dest="*_ssort" />)
puts %(<copyField source="*_isort_stored" dest="*_sort" />)
puts %(<copyField source="*_isort_stored" dest="*_isort" />)
...
puts   %(<dynamicField name="*_a" type="string" indexed="false" stored="true" multiValued="true" />)
puts   %(<dynamicField name="*"   type="string" indexed="false" stored="true" multiValued="false" />)
